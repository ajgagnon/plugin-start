...
use AndreBase\Concerns\Arrayable;
use AndreBaseVendors\PluginEver\QueryBuilder\Query;
...
abstract class Model implements ArrayAccess, JsonSerializable, Arrayable, ModelInterface {
	...
	public function bootModel() {
		...
		if ( ! isset( static::$booted[ $called_class ] ) ) {
			...
			// Call boot on parent classes automatically.
			static::bootParentClasses();

			// Boot traits.
			static::bootTraits();

			// Call the model's boot method.
			static::boot();

			// Call the booted method.
			static::booted();
		}
	}
	 ...
	 * @return void
	 */
	protected static function bootParentClasses() {
		...
		if ( $parents ) {
			foreach ( array_reverse( $parents ) as $parent ) {
				if ( method_exists( $parent, 'boot' ) && self::class !== $parent ) {
	 ...
	 *
	 * @param string|object $class_name Class name or object.
	 * @return array
	 */
	protected static function classUsesRecursive( $class_name ) {
		...
		foreach ( array_reverse( class_parents( $class_name ) ) + [ $class_name => $class_name ] as $single_class ) {
			$results += static::traitUsesRecursive( $single_class );
		}
	...
	/**
	 * Get all traits used by a trait and its parent traits recursively.
	 *
	 * @param string $trait_name Trait name.
	 * @return array
	 */
	protected static function traitUsesRecursive( $trait_name ) {
		...
		foreach ( $traits as $single_trait ) {
			$traits += static::traitUsesRecursive( $single_trait );
		}
	 ...
	 * @return string
	 */
	protected static function classBasename( $class_name ) {
	...
	public static function registerModelEvent( $event, $callback ) {
		...
		// Support multiple callbacks per event (Laravel-style).
		if ( ! isset( static::$events[ $event_name ] ) ) {
			static::$events[ $event_name ] = [];
		}

		static::$events[ $event_name ][] = $callback;
	}

	/**
	 * Register a model observer.
	 * Laravel-style observer registration.
	 *
	 * @param object|string $observer_class Observer class instance or class name.
	 * @return void
	 */
	public static function observe( $observer_class ) {
		$observer = is_string( $observer_class ) ? new $observer_class() : $observer_class;
	...
	public function fireModelEvent( $event ) {
		...
		// Fire global WordPress action.
		\do_action( "andre-base/models/{$called_class}/{$event}", $this );

		// Fire registered callbacks.
		if ( isset( static::$events[ $event_name ] ) ) {
			$callbacks = static::$events[ $event_name ];

			// Support both array of callbacks (new) and single callback (legacy).
			...
			foreach ( $callbacks as $callback ) {
				...
				// If any callback returns false, halt and return false.
				// This allows events like 'creating' or 'saving' to cancel the operation.
				if ( false === $result ) {
					return false;
				}
			}
	...
	public function setAttributes( $attributes, $is_guarded = true ) {
		...
		return apply_filters( "andre-base/{$this->object_name}/set_attributes", $this );
	}
